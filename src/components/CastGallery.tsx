...
export const CastGallery = ({ cast }: { cast: any[] }) => {
    ...
    <View className="mb-20">
      ...
      >
        {cast.slice(0, 10).map((actor) => (
          <View key={actor.id} className="mr-6 md:mr-8 items-center w-20 md:w-24">
            ...
            <Text className="font-sans text-[10px] md:text-xs font-bold text-dark-charcoal text-center" numberOfLines={1}>
              {actor.name}
            </Text>
            <Text className="font-sans text-[9px] md:text-[10px] text-dark-charcoal/50 text-center" numberOfLines={1}>
              {actor.character}
            </Text>
          </View>
        ))}
